...
class ManiSkillBridgeNode(Node):
    ...
    def __init__(self) -> None:
        ...
        # Initialize pose adjustment flag early
        ...
        self.declare_parameter("reset_via_service_on_done", True)
        self.declare_parameter("reset_seed", -1)
        self.declare_parameter("seed", 2)
        # ── Robot base height ──────────────────────────────────────────────
        # robot_base_z: if > 0, enforces this absolute height (in meters).
        # robot_base_z_offset: added to the z that the scene assigns.
        # If both are defined, robot_base_z takes priority.
        self.declare_parameter("robot_base_z", 1.0)       # -1 = disabled
        self.declare_parameter("robot_base_z_offset", 0.0) # 0 = no change
        ...
        self.reset_via_service_on_done = bool(
            self.get_parameter("reset_via_service_on_done").value
        )
        self.reset_seed      = int(self.get_parameter("reset_seed").value)
        self.seed            = int(self.get_parameter("seed").value)
        self.robot_base_z        = float(self.get_parameter("robot_base_z").value)
        self.robot_base_z_offset = float(self.get_parameter("robot_base_z_offset").value)
        ...
        try:
            ...
            raise RuntimeError(
                "Failed to create ManiSkill env."
            ) from exc
        ...
        if not self._pose_adjusted:
            self._apply_robot_base_pose()   # ← applies height after first reset only
        ...
        self.get_logger().info(
            ...
            f"reset_via_service_on_done={self.reset_via_service_on_done} "
            f"reset_seed={self.reset_seed} robot_base_z={self.robot_base_z} "
            f"robot_base_z_offset={self.robot_base_z_offset}"
        )
    ...
    def _apply_robot_base_pose(self) -> None:
        """Adjusts the Z height of the robot base after a reset."""
        if self.robot_base_z < 0 and self.robot_base_z_offset == 0.0:
        ...
        try:
            ...
            # Current position assigned by the scene
            # Convert position to Python native types
            p = current.p
            if hasattr(p, 'tolist'):
                p = p.tolist()
            elif hasattr(p, 'cpu'):
            ...
            if self.robot_base_z >= 0:
                new_z = self.robot_base_z                 # absolute
            else:
                new_z = cz + self.robot_base_z_offset     # relative

            # Convert quaternion to numpy array with float32
            q = current.q
            if hasattr(q, 'numpy'):
                q = q.numpy()
            elif hasattr(q, 'cpu'):
            ...
            # Create numpy arrays with correct dtype for sapien.Pose
            p_array = np.array([cx, cy, new_z], dtype=np.float32)
            
            new_pose = sapien.Pose(p=p_array, q=q)
            robot.set_pose(new_pose)

            self.get_logger().info(
                f"Robot base pose adjusted: [{cx:.3f}, {cy:.3f}, {cz:.3f}] → z={new_z:.3f} m"
            )
        except Exception as exc:
            self.get_logger().warn(f"Could not adjust robot base pose: {exc}")
    ...
    def _tick(self) -> None:
        ...
        for _ in range(max(1, self.sim_steps_per_tick)):
            ...
            if terminated or truncated:
                self.get_logger().info(
                    "Episode finished; waiting for reset service call"
                )
